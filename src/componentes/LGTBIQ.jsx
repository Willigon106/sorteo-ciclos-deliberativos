...
//  const [todosLosGanadores, setTodosLosGanadores] = useState([]);
  ...
  useEffect(() => {
    leerYFiltrarExcel().then(({ filtrados }) => {
      ...
      const filtroLgtbi = filtrados.filter(
        (fila) =>
          fila[24]?.trim().toLowerCase() === "sectores sociales lgbti" 
      );
      setPoblacionLgtbi(filtroLgtbi);
  ...
  const sortear = async(participantes, setGanadores, cupos, e, categoria) => {
    ...
    setPoblacionLgtbi(restantes);

    // Agregar al acumulado global
    /*setTodosLosGanadores((prev) => [
      ...prev,
      { categoria: `LGBTI - Cupo ${count + 1}`, seleccionados }
    ]);*/
...
/*const descargarTXT = () => {
    if (todosLosGanadores.length === 0) {
      alert("No hay seleccionados para descargar.");
      return;
    }

    let contenido = "📋 LISTA DE SELECCIONADOS - Población LGBTI\n\n";
    ...
    todosLosGanadores.forEach((grupo) => {
      ...
      grupo.seleccionados.slice(1).forEach((s, idx) => {
        contenido += `   Suplente ${idx + 1}: ID - ${s[0]}: ${s[5]}\n - ${s[7]}\n`;
      });
      contenido += "\n";
    });
    ...
    enlace.download = "seleccionados-Lgbti.txt";
    enlace.click();
  };*/

  return (
    <div className="contenedor">
      <h1 className="titulo">Sectores Sociales LGBTIQ+</h1>
      <h1 className="titulo"> 3 cupos</h1>
      <h2>Total: {poblacionLgtbi.length}</h2>
      <div className="contenido">
        ...
        <div className="sorteo-container">
            ...
            {ganadoresLgbti.length > 0 && (
              <div className="ganadores">
                <h3>Seleccionados Cupo {count - 1}</h3>
                <p className="titular">
                  <strong>Titular:</strong> {ganadoresLgbti[0][0]} - {ganadoresLgbti[0][5]}<br></br>{ganadoresLgbti[0][7]}
                </p>
                <ul>
                  {ganadoresLgbti.slice(1).map((g, idx) => (
                    <li key={idx}>
