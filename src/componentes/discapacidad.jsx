 ...
 // const [todosLosGanadores, setTodosLosGanadores] = useState([]);
  ...
  useEffect(() => {
    leerYFiltrarExcel().then(({ filtrados }) => {
      ...
      const filtroDiscapacidad = filtrados.filter(
      (fila) =>
        !(
        fila[21]?.trim().toLowerCase() === "ninguna" ||
        fila[21]?.trim().toLowerCase() === "no desea responder"               
      ) &&
      fila[24]?.trim().toLowerCase() === "población con discapacidad"
      );
      setPoblacionDiscapacidad(filtroDiscapacidad);
  ...
  const sortear = async(participantes, setGanadores, cupos, e, categoria) => {
    ...
    setPoblacionDiscapacidad(restantes);

    // Agregar al acumulado global
    /*setTodosLosGanadores((prev) => [
      ...prev,
      { categoria: `Discapacidad - Cupo ${count + 1}`, seleccionados }
    ]);*/
  ...
  return (
    <div className="contenedor">
      <h1 className="titulo">Población con discapacidad</h1>
      <h1 className="titulo"> 3 cupos</h1>
      <h2>Total: {poblacionDiscapacidad.length}</h2>
      <div className="contenido">
        <div className="tabla-container">
          <table className="tabla" border="1" cellPadding="5">
            ...
            <tbody>
              {poblacionDiscapacidad.map((fila, i) => (
                <tr key={i}>
                  <td>{fila[0]}</td>
                  <td>{fila[7]}</td>
                </tr>
        ...
        <div className="sorteo-container">
          ...
          {ganadoresDiscapacidad.length > 0 && (
            <div className="ganadores">
              ...
              <ul>
                {ganadoresDiscapacidad.slice(1).map((g, idx) => (
                  <li key={idx}>
