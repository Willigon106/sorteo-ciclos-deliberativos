...
function App() {
  const [poblacionPalenquera, setPoblacionPalenquera] = useState([]);
...
//  const [todosLosGanadores, setTodosLosGanadores] = useState([]);
  ...
  useEffect(() => {
    leerYFiltrarExcel().then(({ filtrados }) => {
      ...
      const filtroPalenquera = filtrados.filter(
        (fila) =>
          fila[24]?.trim().toLowerCase() === "población palenquera" &&
          fila[19]?.trim().toLowerCase() === "palenquero/a de san basilio"
      );
      setPoblacionPalenquera(filtroPalenquera);
  ...
  return (
        <div className="contenedor">
          <h1 className="titulo">Población Palenquera</h1>
          <h1 className="titulo">1 cupo</h1>
          <h2>Total: {poblacionPalenquera.length}</h2>
          <div className="contenido">
            <div className="tabla-container">
              <table className="tabla">
                ...
                <tbody>
                  {poblacionPalenquera.map((fila, i) => (
                    <tr key={i}>
                      <td>{fila[0]}</td>
                      <td>{fila[7]}</td>
                    </tr>
            ...
            <div className="sorteo-container">
              <div className="boton-container"> <button
                ...
                onClick={(e) => sortear(poblacionPalenquera, setGanadores, 4, e,"Población Palenquera")}
              >
                Comenzar sorteo
              </button></div>
