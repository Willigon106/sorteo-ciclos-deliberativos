 ...
 // const [todosLosGanadores, setTodosLosGanadores] = useState([]);
  ...
  useEffect(() => {
    leerYFiltrarExcel().then(({ filtrados }) => {
      ...
      const filtroAfro = filtrados.filter(
        (fila) =>
  ...
  const sortear = async(participantes, setGanadores, cupos, e, categoria) => {
    ...
    setPoblacionAfro(restantes);

    // Agregar al acumulado global
    /*setTodosLosGanadores((prev) => [
      ...prev,
      { categoria: `Afrocolombianos - Cupo ${count + 1}`, seleccionados }
    ]);*/
  ...
  return (
      <div className="contenedor">
        <h1 className="titulo">Población Afrocolombiana</h1>
        <h1 className="titulo">2 cupos</h1>
        <h2>Total: {poblacionAfro.length}</h2>
          ...
          <div className="sorteo-container">
            ...
            {ganadoresAfro.length > 0 && (
              <div className="ganadores">
                ...
                <ul>
                  {ganadoresAfro.slice(1).map((g, idx) => (
                    <li key={idx}>
